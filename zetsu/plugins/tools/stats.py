...
from zetsu import YouTube, app
from zetsu.core.userbot import assistants
from zetsu.misc import SUDOERS, pymongodb
from zetsu.plugins import ALL_MODULES
from zetsu.utils.database import (get_global_tops,
                                       get_particulars, get_queries,
                                       get_served_chats,
                                       get_served_users, get_sudoers,
                                       get_top_chats, get_topp_users)
from zetsu.utils.decorators.language import language, languageCB
from zetsu.utils.inline.stats import (back_stats_buttons,
                                           back_stats_markup,
                                           get_stats_markup,
        ...
        if what in ["Tracks", "Here"]:
            for items, count in list_arranged.items():
                ...
                if items == "telegram":
                    ...
                    msg += f"💭 [{title}](https://www.youtube.com/watch?v={items}) **Dimainkan {count} Waktu**\n\n"
...
async def overall_stats(client, CallbackQuery, _):
    ...
    text = f"""📊 **Bot Statistik & Informasi:**

✣ **Modul** » {mod}
✣ **Jumlah Kueri** » {total_queries} 
✣ **Jumlah Assisten** » {assistant}
✣ **Pengguna Sudo** » {sudoers} 
✣ **Pengguna Diblokir** » {blocked} 
✣ **Batas Durasi Putar** » {play_duration} Menit
✣ **Batas Unduh Lagu** » {song} Menit 
✣ **Batas Daftar Putar** » {fetch_playlist}
✣ **Durasi Mode Bersih** » {cm} Menit
✣ **Obrolan Yang Dilayani** » {served_chats} 
✣ **Pengguna Yang Dilayani** » {served_users} 
✣ **Assisten Keluar Otomatis** » {ass}
...
✣ **Jumlah Kueri DB** » `{query}`
✣ **Sisa Penyimpanan** » {free[:4]} GB
✣ **Waktu Aktif Mongo** » {mongouptime[:4]} Hᴀʀɪ
✣ **Total Penyimpanan DB** » {storage} MB
✣ **Penyimpanan Tersedia** » {total[:4]} GB
✣ **Penyimpanan Digunakan** » {used[:4]} GB
    """
    med = InputMediaPhoto(media=config.STATS_IMG_URL, caption=text)
    try:
        await CallbackQuery.edit_message_media(
...
async def statszetsu(_, message: Message):
