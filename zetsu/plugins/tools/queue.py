...
from zetsu.utils import (zetsubin, get_channeplayCB,
                              seconds_to_min)
from zetsu.utils.database import (get_cmode, is_active_chat,
                                       is_music_playing)
from zetsu.utils.decorators.language import language, languageCB
from zetsu.utils.inline import queue_back_markup, queue_markup
...
async def queue_back(client, CallbackQuery: CallbackQuery, _):
    ...
    if "live_" in file:
        ...
        if videoid == "telegram":
            ...
            IMAGE = get_image(videoid)
    send = (
        "⏰ **Duration:** **Unknown Duration Stream.**\n\n**Click the button below to get the entire list of queue.**"
        if DUR == "Unknown"
        else "\n**Click the button below to get the entire list of queue.**"
    )
    cap = f"""**{config.MUSIC_BOT_NAME}**
