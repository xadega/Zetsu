...
class Userbot(Client):
    ...
    async def start(self):
        LOGGER(__name__).info(f"Memulai Asisten Klien")
        if config.STRING1:
            ...
            try:
                await self.one.send_message(
                    config.LOG_GROUP_ID, "⚡ **Helper Activated!**"
                )
            except:
        ...
        if config.STRING2:
            ...
            LOGGER(__name__).info(
                f"Asisten Dua Dimulai sebagai {self.two.name}"
            )
        if config.STRING3:
            ...
            LOGGER(__name__).info(
                f"Asisten Tiga Dimulai sebagai {self.three.name}"
            )
        if config.STRING4:
            ...
            LOGGER(__name__).info(
                f"Asisten Empat Dimulai sebagai {self.four.name}"
            )
        if config.STRING5:
            ...
            LOGGER(__name__).info(
                f"Asisten Lima Dimulai sebagai {self.five.name}"
            )
